...
OzoneFileExtension = "_WGS84_.25.25_monthly_Ozone.tif"
OzoneOutputFolder = "D:/10_Article/09_TempOutput/14_MonthlyOzone/"
...
for aimedDate in yearMonthList:
    ...
    for raster in singleMonthRasterNameList:
        ...
        OzoneSingleMonthRasterList.append(singleDayRasterMeanArray)
        
    OzoneMonthlyMeanArray = np.nanmean(OzoneSingleMonthRasterList, axis = 0)
    
    #build the name and address
    OzoneOutputNameFinal = rasterFilePre + aimedDate + OzoneFileExtension
    OzoneOutputRaster = OzoneOutputFolder + OzoneOutputNameFinal
    
    # write the tif files
    driver = gdal.GetDriverByName("GTiff")
    dst_dataset = driver.Create(OzoneOutputRaster, ncol, nrow, nband, gdal.GDT_Float32)
    dst_dataset.SetGeoTransform(geotransform)
    dst_dataset.SetProjection(spatialreference)
    dst_dataset.GetRasterBand(1).WriteArray(OzoneMonthlyMeanArray)
    dst_dataset = None