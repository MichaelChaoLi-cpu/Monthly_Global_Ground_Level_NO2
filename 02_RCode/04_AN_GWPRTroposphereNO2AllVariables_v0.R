...
usedDataset <- usedDataset %>% dplyr::select(
  no2, mg_m2_troposphere_no2, ter_pressure, dayTimeTemperature, nightTimeTemperature, ndvi,
...
formula <- no2 ~ mg_m2_troposphere_no2 + ter_pressure + dayTimeTemperature + nightTimeTemperature + ndvi +
...
GWPR.FEM.bandwidth <- bw.GWPR(formula = formula, data = usedDataset, index = c("CityCode", "period"),
                              ...
                              upperratio = 0.50, effect = "individual", model = "within", approach = "CV",
